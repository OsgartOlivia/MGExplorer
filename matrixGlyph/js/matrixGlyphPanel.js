...
define([], function () {
  ...
  return function MatrixGlyphPanel (matrixGlyphChart) {
	var _matrixGlyphChart = matrixGlyphChart,
	    _width = 180,
		_height = 80,
	    _spanVisibleLines  = null,  // Exibe o número de linhas visiveis no painel
		_divSlider = null,
		_selectLegend = null,
		_selectOrder = null,
    ...
    function _addItemsSelectOrder() {
		...
		if (_matrixGlyphChart.data() != null ) {
			...
			for (i=0; i<sizeValueTitle; i++)
				_selectOrder.append(new Option(_matrixGlyphChart.data().nodes.valueTitle[i], i+1000));  // 100 começa índice numéricos	  
                               // Só funciona quando todos os atributos forem incluidos no <select>			   
