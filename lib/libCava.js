...
//--------------- Adds the edges in the array of edges
            ...
            obj.duoPapersList = function(firstNode, secondNode, graphData) {
                let result = {
                    ...
                    children : {
                        ...
                        valueTitle : graphData.nodes.valueTitle,
                        imageTitle : graphData.nodes.imageTitle,
                        documents: []           // Data of the child nodes and the edge that binds it to the root
                    }
                };
                ...
                graphData.edges.dataEdges.forEach(
                    function (d) {
                        if (d.src === firstNode.idOrig && d.tgt === secondNode.idOrig) {
                            d.documents.forEach( function (doc) {
                                if (isTheFirstOccurence(doc.link, result.children.documents)) {
                                    result.children.documents.push({
                                        type: doc.type,
                                        title: doc.title,
                                        link: doc.link,
                                        date: doc.date,
                        ...
                        if (d.tgt === firstNode.idOrig && d.src === secondNode.idOrig) {
                            d.documents.forEach( function (doc) {
                                if (isTheFirstOccurence(doc.link, result.children.documents)) {
                                    result.children.documents.push({
                                        type: doc.type,
            ...
            obj.clusterPapersList = function(cluster, graphData) {
