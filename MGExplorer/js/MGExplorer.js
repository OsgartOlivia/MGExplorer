require(["dashboard","databaseLib","libCava","algCluster","numericGlyph",
	...
	var ATN_ShortName = 0,   // ATN: Atributo Nodo
		...
		MG_WidthChart = 300;
//		MG_WidthChart = 450;  // Usada para gerar a figura 4.2
...
// ---------------- Acoes de inicializacao
	...
	_historyTree.view = _dashboard
							.configureView({barTitle:true, btTool:false, btClose:false, draggable:true, resizable:true,aspectRatio:true})
							.newView(0,400);					
	...
	_dashboard.configureView({barTitle:true, btTool:true, btClose:false, draggable:true, resizable:true,aspectRatio:true})
	...
	function _showChartRoot( idChart, novo) {
		...
		switch (_selectedQuery) {
			...
			case 5: _loadDataProcess("json/dataUFRGS-2011-CG.json",5,idChart);
					break;
			case 6: _loadDataProcess("json/dataUFRGS-2010-2011.json",6,idChart);
					break;
	...
	function _loadDataProcess(url, codArquivo, idChart) {
		...
		if (_selectedCluster != 0) {
			if (_selectSizeAttrExiste) {
				objChart.chart.panel().removeSelectAttr();
				_selectSizeAttrExiste = false;
			}
		} else {
				if (!_selectSizeAttrExiste) {		
...
//------ Inicializa todos os menus de contexto
...
function _fActionNodeIC_CV(nodeIris, parentId) {
	var vOrder = _dashboard.getChart(parentId).chart.getVOrder();
	var node = _dashboard.getChart(parentId).chart.dataVisToNode(vOrder[nodeIris.indexData]);	
	_showClusterVis(node,parentId);
}
...
//---------------------------------	
	function _showIris(node, parentId) {
		...
		_chart.chart.configCentroid(ATN_ShortName,"coauthors",ATN_ShortName); // Tem que estar antes da função data
